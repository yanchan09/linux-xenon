...
static void _xenon_smc_send(void *msg)
{
	...
	print_hex_dump(KERN_DEBUG, "_xenon_smc_send(): ", DUMP_PREFIX_NONE, 16,
		       2, msg, 16, 0);
...
static void _xenon_smc_wait(void *msg)
{
	...
	wait_event_interruptible(smc.wait_q, smc.cmd == *(unsigned char *)msg);
}
...
static void _xenon_smc_cache(unsigned char *msg)
{
	...
	if (ptr)
		...
		pr_err("unknown smc reply %02x", msg[0]);

	print_hex_dump(KERN_DEBUG, "_xenon_smc_cache(): ", DUMP_PREFIX_NONE, 16,
		       2, msg, 16, 0);
	smc.cmd = msg[0];
}
...
static irqreturn_t xenon_smc_irq(int irq, void *dev_id)
{
	...
	pr_debug("%s = %08x,%08x\n", __func__, irqs, readl(smc.base + 0x94));
...
static int xenon_smc_init_one(struct pci_dev *pdev,
			      const struct pci_device_id *ent)
{
	...
	dev_info(&pdev->dev, "attached to xenon SMC\n");
	...
	if (request_irq(pdev->irq, xenon_smc_irq, IRQF_SHARED, "xenon-smc",
			pdev)) {
		dev_err(&pdev->dev, "xenon-smc: request_irq failed\n");
...
static struct pci_driver xenon_smc_pci_driver = { .name = DRV_NAME,
