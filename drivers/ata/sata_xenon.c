...
static int xenon_init_one(struct pci_dev *pdev,
			  const struct pci_device_id *ent);
static int xenon_scr_read(struct ata_link *link, unsigned int sc_reg, u32 *val);
static int xenon_scr_write(struct ata_link *link, unsigned int sc_reg, u32 val);
...
static unsigned int get_scr_cfg_addr(unsigned int sc_reg)
{
	...
	if ((sc_reg > SCR_CONTROL) || (sc_reg == SCR_ERROR))
		return -1;
...
static int xenon_softreset(struct ata_link *link, unsigned int *classes,
			   unsigned long deadline)
{
	...
	if (pdev->device == 0x5802) {
		...
		dev_warn(&pdev->dev,
			 "Simulating reset of SATA device 0x5802\n");
		return -EIO;
	}
...
static void xenon_bmdma_error_handler(struct ata_port *ap)
{
	ata_do_eh(ap, ata_std_prereset, xenon_softreset, sata_std_hardreset,
		  ata_std_postreset);
}
...
static int xenon_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	ret = dma_set_mask(&pdev->dev, ATA_DMA_MASK);
	if (ret)
		goto err_out_regions;

	ret = dma_set_coherent_mask(&pdev->dev, ATA_DMA_MASK);
	if (ret)
		goto err_out_regions;
