...
struct xenosfb_par {
	...
	struct ati_info __iomem *ati_regs;
};
 ...
 *  != 0 for invalid regno.
 */
static int xenosfb_setcolreg(unsigned int regno, unsigned int red,
...
{
	for (__u32 y = 0; y < rect->height; y++) {
		for (__u32 x = 0; x < rect->width; x++)
...
void xenon_copyarea(struct fb_info *p, const struct fb_copyarea *area)
{
	...
	if ((area->dy == area->sy && area->dx > area->sx) ||
	    (area->dy > area->sy)) {
		for (__s32 y = area->height - 1; y > 0; y--) {
			for (__s32 x = area->width - 1; x > 0; x--) {
				xenon_pset(p, area->dx + x, area->dy + y,
					   xenon_pget(p, area->sx + x,
						      area->sy + y));
			}
		}
	} else {
		for (__u32 y = 0; y < area->height; y++) {
			for (__u32 x = 0; x < area->width; x++) {
				xenon_pset(p, area->dx + x, area->dy + y,
					   xenon_pget(p, area->sx + x,
						      area->sy + y));
			}
...
static const struct fb_ops xenosfb_ops = {
	.owner = THIS_MODULE,
...
static int __init xenosfb_probe(struct platform_device *pdev)
{
	...
	volatile int *gfx;
	struct xenosfb_par *par;

	info = framebuffer_alloc(sizeof(struct xenosfb_par), &pdev->dev);
	if (!info) {
		ret = -ENOMEM;
		return ret;
	}
	...
	if (!par->graphics_base) {
		dev_err(&pdev->dev, "xenosfb: abort, cannot ioremap graphics registers 0x%x @ 0x%llx\n",
	...
	nyres = (vyres - offset_y * 2) * 0x1000 / (scl_v / 0x1000) +
		black_top * 2;

	dev_dbg(&pdev->dev, "virtual resolution: %d x %d\n", vxres, vyres);
	dev_dbg(&pdev->dev, "offset: x=%d, y=%d\n", offset_x, offset_y);
	dev_dbg(&pdev->dev, "black: %d %d, %d %d\n", gfx[0x44 / 4],
		gfx[0x48 / 4], gfx[0x4c / 4], gfx[0x50 / 4]);

	dev_dbg(&pdev->dev, "native resolution: %d x %d\n", nxres, nyres);
	...
	dev_dbg(&pdev->dev, "xenosfb: detected %dx%d framebuffer @ 0x%08x\n",
	...
	/*
	 * size_vmode -- that is the amount of memory needed for the used video
	 * mode, i.e. the minimum amount of memory we need.
	 */
	size_vmode = xenosfb_defined.yres * xenosfb_fix.line_length;

	/*
	 * size_total -- all video memory we have. Used for entries, ressource
	...
	/*
	 * size_remap -- the amount of video memory we are going to use for
	 * xenosfb.  With modern cards it is no option to simply use size_total
	 * as that wastes plenty of kernel address space.
	 */
	...
	if (!request_mem_region(xenosfb_fix.smem_start, size_total,
				"xenosfb")) {
		dev_err(&pdev->dev,
			"xenosfb: cannot reserve video memory at 0x%lx\n",
	...
	info->screen_base = ioremap(xenosfb_fix.smem_start,
				    xenosfb_fix.smem_len);
	if (!info->screen_base) {
		dev_err(&pdev->dev,
			"xenosfb: abort, cannot ioremap video memory 0x%x @ 0x%lx\n",
	...
	dev_dbg(&pdev->dev,
		"xenosfb: framebuffer at 0x%lx, mapped to 0x%p using %dk, total %dk\n",
		xenosfb_fix.smem_start, info->screen_base, size_remap / 1024,
		size_total / 1024);
	dev_dbg(&pdev->dev, "xenosfb: mode is %dx%dx%d, linelength=%d, pages=%d\n",
	...
	xenosfb_defined.yres_virtual =
		xenosfb_fix.smem_len / xenosfb_fix.line_length;
	dev_dbg(&pdev->dev, "xenosfb: scrolling: redraw\n");
	...
	dev_dbg(&pdev->dev, "xenosfb: pixclk=%ld left=%02x hsync=%02x\n",
		(unsigned long)xenosfb_defined.pixclock,
		xenosfb_defined.left_margin,
	...
	dev_dbg(&pdev->dev, "xenosfb: Truecolor: size=%d:%d:%d:%d, shift=%d:%d:%d:%d\n",
		screen_info.rsvd_size, screen_info.red_size,
		screen_info.green_size, screen_info.blue_size,
		screen_info.rsvd_pos, screen_info.red_pos,
	...
	/*
	 * request failure does not faze us, as vgacon probably has this region
	 * already (FIXME)
	 */
...
static int __exit xenosfb_remove(struct platform_device *dev)
{
	...
	if (info) {
		struct xenosfb_par *par = info->par;
