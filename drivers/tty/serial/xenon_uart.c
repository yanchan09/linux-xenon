...
static void xenon_set_termios(struct uart_port *port, struct ktermios *new,
...
static const struct uart_ops xenon_ops = {
...
static int __init xenon_init(void)
{
	...
	pr_info("Xenon XBOX 360 serial driver\n");
...
{
	pr_info("Xenon XBOX 360 serial driver exit\n");
 ...
 */
static void xenon_console_write(struct console *cons, const char *s,
				unsigned int count)
{
