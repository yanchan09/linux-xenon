...
static void xenosrb_reset_ring(void)
{
	...
	out_be32(xenosrb_info.graphics_base + 0x071C,
		 0x00000000); /* CP_RB_RPTR_WR */
	out_be32(xenosrb_info.graphics_base + 0x0714,
		 0x00000000); /* CP_RB_WPTR */
	udelay(1000);
...
static int xenosrb_upload_pfp_ucode(const uint32_t *ucode, size_t size)
{
	...
	out_be32(xenosrb_info.graphics_base + 0x117C,
		 0); /* CP_PFP_UCODE_ADDR */
	udelay(100);
...
static int xenosrb_upload_me_ucode(const uint32_t *ucode, size_t size)
{
	...
	for (i = 0; i < size; i++) {
		if (in_be32(xenosrb_info.graphics_base + 0x07E8) != ucode[i]) {
			pr_err("%s: failed to verify me microcode @ dword %zu\n",
			       __func__, i);
			return -EIO;
		}
	}
...
static int xenosrb_setup(void *buffer_base, size_t buffer_size)
{
	...
	out_be32(xenosrb_info.graphics_base + 0x07D8,
		 0x1000FFFF); /* CP_ME_CNTL.ME_HALT = 1 */
	udelay(1000);

	xenosrb_reset_ring();
	xenosrb_setup_ring(buffer_base, buffer_size);

	ret = xenosrb_upload_pfp_ucode(xenos_pfp_ucode,
				      ARRAY_SIZE(xenos_pfp_ucode));
	if (ret)
		return ret;

	ret = xenosrb_upload_me_ucode(xenos_me_ucode,
	...
	out_be32(xenosrb_info.graphics_base + 0x07D8,
		 0x0000FFFF); /* CP_ME_CNTL.ME_HALT = 0 */
	out_be32(xenosrb_info.graphics_base + 0x07D0,
		 0x0000FFFF); /* CP_INT_ACK.RTS[0..15]ACK = 1 */
	out_be32(xenosrb_info.graphics_base + 0x07F0,
		 0x00000000); /* CP_DEBUG */
	out_be32(xenosrb_info.graphics_base + 0x07D8,
		 0x1000FFFF); /* CP_ME_CNTL.ME_HALT = 1 */

	udelay(2000);
	out_be32(xenosrb_info.graphics_base + 0x00F0,
		 0x00000001); /* RBBM_SOFT_RESET.SOFT_RESET_CP = 1 */
	udelay(1000);
	out_be32(xenosrb_info.graphics_base + 0x00F0,
		 0x00000000); /* RBBM_SOFT_RESET.SOFT_RESET_CP = 0 */
	udelay(1000);

	ret = xenosrb_wait_gui_idle(HZ / 4);
	if (ret) {
		pr_err("%s: timed out waiting for GUI idle (reset)\n",
		       __func__);
		return ret;
	}
	...
	ret = xenosrb_wait_gui_idle(HZ / 4);
	if (ret) {
		pr_err("%s: timed out waiting for GUI idle (final)\n",
		       __func__);
		return ret;
	}
...
{
	out_be32(xenosrb_info.graphics_base + 0x07D8,
		 0x10000000); /* CP_ME_CNTL.ME_HALT = 1 */
	out_be32(xenosrb_info.graphics_base + 0x0704,
		 0x00000000); /* CP_RB_CNTL */
	out_be32(xenosrb_info.graphics_base + 0x0700,
		 0x00000000); /* CP_RB_BASE */
	xenosrb_reset_ring();
}
			     ...
			     size_t count, loff_t *ppos)
{
	...
	if (space <= count) {
		...
		primary_rb->tail =
			in_be32(xenosrb_info.graphics_base + 0x0710) / 4;
...
static struct miscdevice xenosrb_dev = { .minor = MISC_DYNAMIC_MINOR,
					 "xenosrb",
					 &xenos_fops };
			 ...
			 const struct pci_device_id *dev_id)
{
	...
	ret = pci_request_regions(dev, dev->dev.kobj.name);
	...
	if (!xenosrb_info.graphics_base) {
		dev_err(&dev->dev, "%s: failed to ioremap gfx regs\n",
			__func__);
		ret = -EIO;
		goto err_release_pci_regions;
	}

	ret = misc_register(&xenosrb_dev);
	if (ret)
	...
	ret = request_irq(dev->irq, xenos_interrupt, IRQF_SHARED, "Xenos", dev);
	if (ret)
		dev_err(&dev->dev, "%s: failed to request IRQ 0x%.2X\n",
			__func__, dev->irq);

	dev_info(&dev->dev,
		 "XenosRB Character Driver Initialized, ring size = %d\n",
		 xenosrb_info.rb_size);
	return 0;
...
static int __init xenosrb_init(void)
{
	...
	ret = pci_register_driver(&xenosrb_pci);
	if (ret)
		pr_err("%s: pci_register_driver failed with %d", __func__, ret);
