...
static void console_clrscr(struct console_context *context)
{
	unsigned int *fb = (unsigned int *)context->fb;
	int count = context->width * context->height;

	while (count--)
		out_be32(fb++, 0x00000000);
...
static inline void console_pset32(struct console_context *context, int x, int y,
				  int color)
{
...
static inline void console_pset(struct console_context *context, int x, int y,
...
static void console_draw_char(struct console_context *context,
	...
	for (int ly = 0; ly < 16; ly++) {
		for (int lx = 0; lx < 8; lx++)
			console_pset32(context, x + lx, y + ly,
				       font_pixel(c, lx, ly) == 1 ? 0xFFFFFF00 :
								    0x00000000);
	}
#undef font_pixel
}

static void console_scroll32(struct console_context *context,
			     const unsigned int lines)
{
	...
	for (int l = lines; l * 32 < context->height; l++)
		memcpy(context->fb + bs * (l - lines), context->fb + bs * l,
		       bs);
...
static void console_newline(struct console_context *context)
{
...
void __init udbg_init_xenon(void)
{
	 ...
	 * set up the screen.
	 */
	memset(&console_ctx, 0, sizeof(struct console_context));
